...
Deno.test({
    ...
    fn() {
        ...
        assertEquals(left<number, string>(1).right.map((s: string) => s.toUpperCase()).try(), 1);
        assertEquals(left<number, string>(1).right.map((s: string) => s.toUpperCase()).get(), 1);
        assertEquals(left<number, string>(1).right.coerce(parseFloat), 1);
        assertEquals(left<number, string>(1).mapEither((n: number) => n.toString(), (s: string) => s), '1')
...
Deno.test({
    ...
    fn() {
        ...
        assertEquals(JSON.stringify(jsonError(error)), JSON.stringify({name: error.name, message: error.message, stack: error.stack}));
    }
})
...
Deno.test({
    ...
    fn() {
        const [leftValues, rightValues] = partition<number, string>([left(1), left(3), right('hello'), left(0.5), right('0.5'), right('one'), left(NaN)]);
...
Deno.test({
    ...
    async fn() {
        ...
        assertEquals((await tryCatchAsync<string, Error>(async () => 'hello')).try(), 'hello');
        assertThrows(() => tryCatch<string, Error>(() => { throw new Error('error') }).try());
        assertThrowsAsync(async () => {
            const valueOrError = await tryCatchAsync<string, Error>(() => Promise.reject(new Error('error')));
            valueOrError.try();
        });
    }
})
